  ...
  if (request.method === "POST" && !eventId) {
    ...
    const [event] = await sql`
      ...
      values (
        ...
        ${input.heroImageUrl},
        ${input.questionCount},
        ${JSON.stringify(input.targetMedia)}::jsonb,
        ${JSON.stringify(input.rewardMetadata)}::jsonb,
        ${input.isActive},
        ${input.difficulty},
  ...
  if (request.method === "PATCH" && eventId) {
    ...
    const [event] = await sql`
      ...
      set
        ...
        target_media = ${JSON.stringify(input.targetMedia)}::jsonb,
        reward_metadata = ${JSON.stringify(input.rewardMetadata)}::jsonb,
