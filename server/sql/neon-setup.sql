...
create unique index if not exists season_release_tracking_show_season_unique
  on season_release_tracking (tmdb_show_id, season_number);

create index if not exists season_release_tracking_media_idx
  on season_release_tracking (media_item_id, last_checked_at);
...
create unique index if not exists episode_release_tracking_show_episode_unique
  on episode_release_tracking (tmdb_show_id, season_number, episode_number);

create index if not exists episode_release_tracking_release_idx
  on episode_release_tracking (release_date, status);
...
alter table friend_trivia_challenges
...
create unique index if not exists friend_trivia_challenges_token_unique
  on friend_trivia_challenges (token);

create index if not exists friend_trivia_challenges_user_created_idx
  on friend_trivia_challenges (challenger_user_id, created_at desc);

create index if not exists friend_trivia_challenges_title_idx
  on friend_trivia_challenges (media_type, tmdb_id, created_at desc);
...
create table if not exists friend_trivia_attempts (
  ...
  result text not null
    check (result in ('won', 'lost', 'tie')),
  answers jsonb not null default '{}'::jsonb,
  completed_at timestamptz not null default now(),
  created_at timestamptz not null default now()
);

create index if not exists friend_trivia_attempts_challenge_score_idx
  on friend_trivia_attempts (challenge_id, score desc, completed_at desc);

create index if not exists friend_trivia_attempts_user_created_idx
  on friend_trivia_attempts (user_id, created_at desc);
...
create index if not exists affiliate_mappings_provider_region_idx
  on affiliate_mappings (provider_id, region, active);
...
create index if not exists ticket_providers_region_status_idx
  on ticket_providers (region, status);
...
create index if not exists title_ticket_availability_media_region_idx
  on title_ticket_availability (media_item_id, region, status);

create index if not exists title_ticket_availability_showtime_idx
  on title_ticket_availability (showtime_date);
...
create index if not exists ticket_affiliate_links_media_region_idx
  on ticket_affiliate_links (media_item_id, region, active);
...
create table if not exists ad_targeting_rules (
  ...
  genre_context text,
  media_type_context text
    check (media_type_context in ('movie', 'tv') or media_type_context is null),
  region text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create index if not exists ad_targeting_rules_campaign_idx
  on ad_targeting_rules (campaign_id);
...
create index if not exists ad_placements_type_status_dates_idx
  on ad_placements (placement_type, status, start_date, end_date);
...
create index if not exists ad_impressions_campaign_time_idx
  on ad_impressions (campaign_id, occurred_at desc);

create index if not exists ad_impressions_placement_time_idx
  on ad_impressions (placement_id, occurred_at desc);
...
create index if not exists ad_clicks_campaign_time_idx
  on ad_clicks (campaign_id, clicked_at desc);
...
create index if not exists arcade_wallets_updated_at_idx
  on arcade_wallets (updated_at desc);
...
create unique index if not exists arcade_transactions_user_idempotency_unique
  on arcade_transactions (user_id, idempotency_key)
  where idempotency_key is not null;

create index if not exists arcade_transactions_user_created_idx
  on arcade_transactions (user_id, created_at desc);

create index if not exists arcade_transactions_type_created_idx
  on arcade_transactions (transaction_type, created_at desc);
...
create index if not exists arcade_daily_token_grants_status_dates_idx
  on arcade_daily_token_grants (status, starts_at, ends_at);
...
create index if not exists arcade_game_pricing_game_status_idx
  on arcade_game_pricing (game_id, pricing_status);

create index if not exists arcade_game_pricing_instance_status_idx
  on arcade_game_pricing (game_instance_id, pricing_status);

create index if not exists arcade_game_pricing_key_status_idx
  on arcade_game_pricing (game_key, pricing_status);
...
create index if not exists concession_reward_categories_status_order_idx
  on concession_reward_categories (status, display_order);
...
create index if not exists concession_rewards_status_dates_idx
  on concession_rewards (status, starts_at, ends_at);

create index if not exists concession_rewards_type_status_idx
  on concession_rewards (reward_type, status);

create index if not exists concession_rewards_category_idx
  on concession_rewards (category_id, status);
...
create unique index if not exists user_reward_inventory_active_unique
  on user_reward_inventory (user_id, reward_id)
  where ownership_status = 'active';

create index if not exists user_reward_inventory_user_status_idx
  on user_reward_inventory (user_id, ownership_status, acquired_at desc);
...
create unique index if not exists reward_redemptions_user_idempotency_unique
  on reward_redemptions (user_id, idempotency_key)
  where idempotency_key is not null;

create index if not exists reward_redemptions_user_status_idx
  on reward_redemptions (user_id, status, created_at desc);

create index if not exists reward_redemptions_reward_status_idx
  on reward_redemptions (reward_id, status, created_at desc);
...
create index if not exists arcade_fraud_events_user_status_idx
  on arcade_fraud_events (user_id, status, created_at desc);

create index if not exists arcade_fraud_events_type_severity_idx
  on arcade_fraud_events (event_type, severity, created_at desc);
...
create index if not exists arcade_economy_daily_metrics_type_date_idx
  on arcade_economy_daily_metrics (metric_type, metric_date desc);
